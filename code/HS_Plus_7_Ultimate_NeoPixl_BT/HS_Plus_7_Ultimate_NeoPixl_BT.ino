// ###################################################################################
// # Project: HS_Plus_7_Ultimate_NeoPixl
// # Engineer:  Mic.Tsai
// # Date:  20 May 2014
// # Objective: Dev.board
// # Usage: ATMEL328P
...
#define N_LEDS       50 // Max value is 170 (fits one SD card block)
...
void setup() {
  ...
  if(maxLPS > 300) maxLPS = 340; // NeoPixel PWM rate is ~400 Hz //change
  Serial.print(F("Max lines/sec: "));
  int second2 = maxLPS*10;
...
//============================================================================== SET POINT
//=================================  0.002925  0.0029 0.00295 big-fast

void Setpoint()
{
    //0  BLACK
    bmpDraw(4310);    //frame number
...
void BreatheWhite(unsigned int x)
{
  for (; x>0; x--)
  {
    for (int BreatheNum = 0; BreatheNum < 255 ; BreatheNum++ )
    {
      for (int i=0; i<LED_COUNT; i++)
      {
    ...
    for (int BreatheNum = 255; BreatheNum > 0 ; BreatheNum-- )
    {
      for (int i=0; i<LED_COUNT; i++)
      {
        leds.setPixelColor(i, BreatheNum , BreatheNum , BreatheNum );
